...
nUEs = 4; %Total Users
Ns=1;
nNts =16:16:300; % Total BS Antennas
No=1;
var_db = [0,-9.7,-19.2,-22.8,-30]; %Ped-A Channel
%var_db = [1,1,1,1,1]; %Rayleigh Fading Channel
var = 10.^(var_db/10);
Ch_taps=length(var_db); % Total Channel Taps
SNR_db = 15;SNR = 10.^(SNR_db/10);
uu_itrns=7;lrg_itrns=7;sml_itrns=7;

for it=1:lrg_itrns
    for jj=1:sml_itrns
        htotl{it,jj}=(randn(nUEs,nNts(end),Ch_taps)+1j.*randn(nUEs,nNts(end),Ch_taps)).*(reshape(sqrt(1/2),1,1,[]));
    end
end
...
for jj=1:sml_itrns
    for rr=1:length(nNts)
        [precoder,uu,it,jj,rr]
        htemp=htotl{it,jj};
        hpre=htemp(:,1:nNts(rr),:);
       ...
       %      end
        ...
        elseif precoder==2
            for m=1:nSubC
                P1(:,:,m)=inv((Hm(:,:,m)'*Hm(:,:,m)+(No/SNR)*eye(nNts(rr))))*(Hm(:,:,m)');
            end
        end
        ...
        Srate(jj,rr) = sum(SUM_rate_avr)/nSubC;
    end
end
...
plot(nNts,SE(1,:),'LineWidth', 1.5, 'Marker', 'square', 'MarkerSize', 6 , 'MarkerFaceColor','g');
hold on;
plot(nNts,SE(2,:),'LineWidth', 1.5, 'Marker', 'square', 'MarkerSize', 6 , 'MarkerFaceColor','c');
legend('MRT','MMSE','Location','northwest');
xlabel("No of Antennas at BS");
